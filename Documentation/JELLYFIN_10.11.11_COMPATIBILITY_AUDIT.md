...
| Automatic refresh | Displays saved item snapshots without waiting for live rules. Rotating, seasonal, and cross-user activity sections re-evaluate only while Home is active, through the two-request Home lane; activity results also use a 60-second server cache. Periodic all-rule polling is deliberately not performed by the browser client because it previously coupled page responsiveness to background library work. | Safe read-only browser boundary. A future server-side library-event implementation is required before the newly-added-media toggle can be described as runtime-complete. |

## Recovery release boundary

Version 0.1.0.18 remains the emergency recovery package that disables the browser runtime. Version 0.1.0.19 restores the requested browser features using the route isolation, bounded request lanes, independent cached rendering, and playback restrictions documented above.
