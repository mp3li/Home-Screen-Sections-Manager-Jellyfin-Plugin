...
| Custom home rows | Serves embedded authenticated client settings and browser assets from plugin controller routes, waits for the cached `#indexPage` and active `#homeTab`, restores rows independently from the Abyss frame after DOM replacement, limits Home reads to two concurrent requests, and pages saved or dynamic content 40 items at a time through Jellyfin 10.11.11’s registered `emby-scroller` and `emby-itemscontainer` elements. | Supported plugin-controller, cached-page lifecycle, and native scroller pattern; live Jellyfin Web behavior still requires server testing. |
