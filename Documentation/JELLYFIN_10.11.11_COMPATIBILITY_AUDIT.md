...
| Custom home rows | Serves embedded authenticated client settings and browser assets from plugin controller routes, paints saved/cache-backed rows before display-preferences or media-bar work finishes, limits Home reads to two concurrent requests, and loads 40 saved IDs at a time through Jellyfin 10.11.11's registered `emby-scroller` and `emby-itemscontainer` elements. | Supported plugin-controller and native scroller pattern; live Jellyfin Web behavior still requires server testing. |
...
| My List | Inserts its button in Jellyfin's native `.emby-tabs-slider`, adds a matching Home page tab panel, and uses official `UserData.Likes`, `Filters=Likes`, and Jellyfin Web 10.11.11's boolean `updateUserItemRating(userId, itemId, likes)` contract. Visible hearts begin from cache and hydrate through one bounded batch lane; a click performs one explicit rating write and caches the returned item. Episode entries retain the episode Likes value while displaying the series name/art as their primary card identity. | Per-user Jellyfin data; no separate plugin media database. |
...
| Library identity and native Latest Media | Uses current user views and stable Jellyfin IDs first, stores server/user-scoped ID/name/type snapshots, migrates only unambiguous replacement IDs, repairs stale `topParentId` routes, resolves native rows by their `HomeSectionType` token, and treats `latestmedia` as one native wrapper without reading, caching, or rebuilding its inner per-library Recently Added rows. | Conservative client-side identity reconciliation. Ambiguous delete/re-add replacements require administrator selection rather than a guessed mapping. |

## Recovery release boundary

Version 0.1.0.18 remains the emergency recovery package that disables the browser runtime. Version 0.1.0.20 retains the 0.1.0.19 route isolation and playback restrictions while adding live settings reconciliation, explicit manual rule refresh, and conservative library identity repair.
