...
| Automatic refresh | Displays saved item snapshots first, then re-queries saved collection/library sources and, when Collection Manager is installed, its existing metadata preview routes for saved tag rules every 60 seconds. Rotating and seasonal sections re-evaluate the active draft; activity sections re-read recent/current activity. | Supported companion-plugin and official read-only workflow; version-pair and multi-user testing required. |
