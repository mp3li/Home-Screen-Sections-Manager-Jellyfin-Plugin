...
| My List | Inserts its button in Jellyfin's native `.emby-tabs-slider`, adds a matching Home page tab panel, and uses official `UserData.Likes`, `Filters=Likes`, and `updateUserItemRating(userId, itemId, bool)` contracts. | Per-user Jellyfin data; no separate plugin media database. |
